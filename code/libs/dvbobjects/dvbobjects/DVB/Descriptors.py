...
class parental_rating_descriptor(Descriptor):  #!                                                                                                                                            
    ...
    def bytes(self):
        ...
        fmt = "!%dsB" % (len(self.country_code))

        return pack(fmt, self.country_code, self.rating)
...
class stream_identifier_descriptor(Descriptor):#!
    ...
    def bytes(self):
        ...
        return pack(fmt, self.component_tag)
...
#         return pack(fmt, self.bouquet_name)
...
class data_broadcast_id_descriptor(Descriptor):#!
    ...
    def bytes(self):
        ...
        if (self.data_broadcast_ID == 0x000A): # DVB-SSU
            ...
            fmt = "!HB%ds%ds" % (oui_data_length, 
                len(self.private_data_bytes))
...
class local_time_offset_loop_item(DVBobject):#!
    ...
    def pack(self):
        ...
        return pack(fmt,
            self.ISO_639_language_code,
            ((self.country_region_id & 0x3F) << 2) | 0x2 | 
            (self.local_time_offset_polarity & 0x1),
...
######################################################################
class multilingual_network_descriptor_loop_item(DVBobject):

    def pack(self):
        fmt = "!3s%ds" % len(self.network_name)
...
class multilingual_network_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, multilingual_network_bytes)
...
class transport_stream_terrestrial_descriptor(Descriptor):
   ...
   def bytes(self):
       ...
       return pack(fmt,
           ...
           (self.hierarchy_information << 3)| 
           (self.code_rate_HP_stream),
           (self.code_rate_LP_stream << 5) | 
    ...
    def bytes(self):
        ...
        if self.modulation_system == 1:
            ...
            return pack(fmt,
                ...
                (self.polarization << 5) | 
                (self.roll_off << 3) | 
                (self.modulation_system << 2) |
                self.modulation_type,
                (self.symbol_rate << 4) | self.FEC_inner)

        elif self.modulation_system == 0:
...
class service_descriptor_loop_item(DVBobject):
    ...
    def pack(self):
        ...
        return pack(fmt, self.service_ID, self.service_type)
...
class service_list_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, dvb_service_bytes)
...
class PDC_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt,
            (0xF0) | (self.day >> 1),
            (self.day << 15) | (self.month << 11) | 
            (self.hour << 6) | (self.minute))
...
class target_serial_number_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.serial_data_bytes)
...
class subtitling_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, len(data_bytes), data_bytes)
...
class target_IP_source_slash_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.IPv4_source_dest_bytes)   
...
class target_IPv6_slash_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.IPv6_bytes)   
...
class target_IPv6_source_slash_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.IPv6_source_dest_bytes)   
...
class isp_access_mode_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.access_mode)
...
class telephone_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        fmt = "!BBBB%ds%ds%ds%ds%ds" % (
            len(country_prefix_bytes), 
            len(international_area_code_bytes), 
            len(operator_code_bytes), 
            len(national_area_code_bytes), 
            len(core_number_bytes))
    
        return pack(fmt,
            (0x03 << 7) & 0xC0 | (self.foreign_availability << 5) & 
            0x20 | self.connection_type & 0x1F,
            (0x01 << 7) & 0x80 | (self.country_prefix_length << 5) & 
            0x60 | (self.international_area_code << 4) & 
            0x1C | self.operator_code_length & 0x07,
            (0x01 << 7) & 0x80 | (self.national_area_code_length << 4) & 
            0x70 | self.core_number_length & 0x0F, 
...
class private_data_specifier_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.private_data_specifier)
...
class time_slice_fec_identifier_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt,
            (self.time_slicing << 7) & 0x80 | 
            (self.mpe_fec << 5) & 0x60 | (0x03 << 3) | self.frame_size & 0x07,
...
class OUI_info_loop_item(DVBobject):
  ...
  def pack(self):
    ...
    return pack(fmt,
        ...
        0xC0 | (self.update_versioning_flag & 0x1) << 5 | 
        self.update_version & 0x1F,
        len(self.selector_bytes),
        self.selector_bytes)
...
class terrestrial_delivery_system_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt,
            ...
            (self.bandwidth << 5) & 0xE0 | 
            (self.priority << 4) & 0x10 | 
            (self.timeslice_ind << 3) & 0x08 |
...
class scheduling_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt,
            MJD_convert(self.year_of_start_date_time, 
                self.month_of_start_date_time, 
                self.day_of_start_date_time),
            self.hour_of_start_date_time,
            self.minute_of_start_date_time,
            self.second_of_start_date_time,
            MJD_convert(self.year_of_end_date_time, 
                self.month_of_end_date_time, 
                self.day_of_end_date_time),
            self.hour_of_end_date_time,
            self.minute_of_end_date_time,
            self.second_of_end_date_time,
            self.final_availability << 7 | 
            self.Periodicity_flag << 6 | 
            self.period_unit << 4 | 
            self.duration_unit << 2 | 
            self.estimated_cycle_time_unit,
            self.Period,
            self.Duration,
...
class default_authority_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt, self.authority)
...
class dts_audio_stream_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt,
            ((self.sample_rate_code & 0xF) << 4) | 
            ((self.bit_rate_code >> 2) & 0xF),
            ((self.bit_rate_code & 0x3) << 6) | 
            ((self.nblks >> 1) & 0x3F),
            ((self.nblks & 0x1) << 7) | 
            ((self.fsize >> 7) & 0x7F),
            ((self.fsize & 0x7F) << 1) | 
            ((self.surround_mode >> 5) & 0x1),
            ((self.surround_mode & 0x1F) << 3) | 
            ((self.lfe_flag & 0x1) << 2) | 
            (self.extendend_surround_flag & 0x3),
            self.additional_info)
...
class ISO_639_language_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        fmt = "!%dsB" % (len(self.ISO_639_language_code))
...
class supplementary_audio_descriptor(Descriptor):
    ...
    def bytes(self):
        ...
        return pack(fmt,
            ...
            (self.mix_type & 0x1) << 7 | 
            (self.editorial_classification & 0x1F) << 2 | 0x3,
            self.ISO_639_language_code,
            self.private_data_bytes)
