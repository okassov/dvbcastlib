...
###############################
# Split DVB Tables to section #
###############################

def split_to_section_bat(items_list, parts=2):

    result = []

    bat_object_id = items_list["bat"]
    bouquet_id = items_list["bouquet_id"]
    descriptors = items_list["descriptors"]

    transports = items_list["transports"]

    length = len(transports)

    split_items = [ transports[i*length // parts: (i+1)*length // parts] for i in range(parts) ]

    for chunk in split_items:
        ...
        result.append( 
            { 
                ...
                "transports": [ transport for transport in chunk ]
            } 
        )
...
def split_to_section_nit(items_list, parts=2):

    result = []

    network_object_id = items_list["network"]
    network_id = items_list["network_id"]
    descriptors = items_list["descriptors"]
...
def split_to_section_sdt(items_list, parts=2):
    ...
    split_items = [ 
        items_list[ 
            i*length // parts: (i+1)*length // parts 
...
def event_chunks_list(alist, parts=2):
    ...
    chunks = [ 
        alist[ 
            i:i+parts 
        ] for i in range(0, len(alist), parts) 
    ]
...
###################################
# DVB Table descriptor generators #
###################################


def bat_descriptors_generator(descriptor):
    ''''''

    handlers = {
        "bouquet_name_descriptor" : bouquet_name_descriptor_func,
        ...
        "nds_e2_descriptor": nds_e2_descriptor_func,
        "nds_e4_descriptor": nds_e4_descriptor_func
    }
    ...
    if descriptor_name in handlers:
        ...
        print ("Error! Unknown BAT descriptor.")
        result = None
    return result
...
############################
# Section length calculate #
############################


def section_length_calculate(first_loop, second_loop = []):
    '''This function calculate section length.
    Get first_loop descriptors and optional second_loop 
...
#############################
# DVB Table loop generators #
#############################

def bat_loops(bat_data, bouquet_id, network_id):
    ...
    Return:
    (section_length_calculate, bdl, tdl)'''

    bouquet_descriptor_loop = []
    transport_stream_loop = []

    first_loop_descriptors = bat_data["descriptors"]

    # Generate first loop
    for descriptor in first_loop_descriptors:
        descriptors_pack = bat_descriptors_generator(descriptor)
        if descriptors_pack != None:
            bouquet_descriptor_loop.append(descriptors_pack)
        else:
            pass

    # Generate second loop
    for transport in bat_data["transports"]:
    ...
    return (section_length_calculate(
                bouquet_descriptor_loop, 
                transport_stream_loop
            ), 
            bouquet_descriptor_loop, 
...
def sdt_loops(services):
    ...
    return section_length_calculate(service_loop), service_loop
...
##########################
# DVB Table check length #
##########################

def check_length(item_length, items_list, dvb_table, *args, **kwargs):
    ...
    Finally return list with sections of dvb_table with
    standard length'''
    ...
    if 0 <= (item_length + 13) <= (section_max_size - 3):
        ...
        if dvb_table == "BAT":
            sections = split_to_section_bat(items_list)
        elif dvb_table == "NIT":
            sections = split_to_section_nit(items_list)
        else:
            pass
...
def check_length_sdt(item_length, items_list, transport_id, dvb_table):
    '''This function check length of second loop for all
...
def check_length_eit(items_list, dvb_table):
    ...
    if dvb_table == "EIT Actual Schedule":
        ts_section_list = eit_sched_for_sections
    elif dvb_table == "EIT_Actual_PF":
        ts_section_list = eit_act_pf_for_sections
    elif dvb_table == "EIT_Other_PF":
